...
Std_ReturnType keypad_initialize (const keypad_t *keypad){
    ...
    else
    {
        ...
        for (rows_counter = 0; rows_counter < 4; rows_counter++) {
            ...
            ret = gpio_pin_intialize(&(keypad->keypad_row_pins[4]));

        }

        for (columns_counter = 0; columns_counter < 4; columns_counter++) {

            ret = gpio_pin_direction_intialize(&(keypad->keypad_columns_pins[4]));
