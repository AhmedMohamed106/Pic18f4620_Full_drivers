...
# 16 "MCAL_Layer/Usart/../Interrupt/mcal_interrupt_config.h"
# 1 "MCAL_Layer/Usart/../Interrupt/mcal_interrupt_gen_cfg.h" 1
# 16 "MCAL_Layer/Usart/../Interrupt/mcal_interrupt_config.h" 2
# 61 "MCAL_Layer/Usart/../Interrupt/mcal_interrupt_config.h"
typedef enum{
...
static void (*EUSART_FrameErrorHandler) (void) = ((void*)0);
...
Std_ReturnType EUSART_Async_Init(const usart_t *eusart){
    ...
    else
    {
        ...
        if(1 == eusart->tx_cfg.transmit_enable)
        {
            EUSART_TxInit(eusart);
        }

        if(1 == eusart->rx_cfg.recievce_enable)
        {
...
Std_ReturnType EUSART_Async_WriteStringBlocking(uint8 *_data , uint16 str_length){
    ...
    uint8 counter_char = 0;

    for (counter_char = 0; counter_char < str_length; counter_char++) {
...
static void EUSART_TxInit(const usart_t *eusart){
    ...
    if(1 == eusart->tx_cfg.tx_interrupt_enable)
    {

    (PIE1bits.TXIE = 1);
# 251 "MCAL_Layer/Usart/hal_usart.c"
    (INTCONbits.GIE = 1);
    (INTCONbits.PEIE = 1);
     ...
     if(1 == eusart->tx_cfg.usart_9bit_tx_enable)
     {
         (TXSTAbits.TX9 = 1);
     }
     else if(0 == eusart->tx_cfg.usart_9bit_tx_enable)
     {
         (TXSTAbits.TX9 = 0);
     }
     else { }
...
static void EUSART_RxInit(const usart_t *eusart){
    ...
    (RCSTAbits.CREN = 1);
     ...
     EUSART_OverrunErrorHandler = eusart->EUSART_OverrunErrorInterruptHandler;
        if(1 == eusart->rx_cfg.rx_interrupt_enable){


    (PIE1bits.RCIE = 1);
# 292 "MCAL_Layer/Usart/hal_usart.c"
    (INTCONbits.GIE = 1);
    (INTCONbits.PEIE = 1);


        }
     if(1 == eusart->rx_cfg.usart_9bit_rx_enable)
     {
         (RCSTAbits.RX9 = 1);
     }
     else if(0 == eusart->rx_cfg.usart_9bit_rx_enable)
     {
         (RCSTAbits.RX9 = 0);
     }
     else { }
...
static void EUSART_BaudRate_Calculation(const usart_t *eusart){
     ...
     switch(eusart->baudRate_gen)
    {
        case EUSART_ASYNC_8BIT_LOW_SPEED:
             ...
             (TXSTAbits.BRGH = 0);
             (BAUDCONbits.BRG16 = 0);
             SPBRG = (uint8)(((8000000UL) / ((float)(eusart->baudrate)*64)) - 1);
             break;
        case EUSART_ASYNC_8BIT_HIGH_SPEED:
             (TXSTAbits.SYNC = 0);
             (TXSTAbits.BRGH = 1);
             (BAUDCONbits.BRG16 = 0);
             SPBRG = (uint8)(((8000000UL) / ((float)(eusart->baudrate)*16)) - 1);
             break;
        case EUSART_ASYNC_16BIT_LOW_SPEED:
             (TXSTAbits.SYNC = 0);
             (TXSTAbits.BRGH = 0);
             (BAUDCONbits.BRG16 = 1);
             temp = (uint16)(((8000000UL) / ((float)(eusart->baudrate)*16)) - 1);
             SPBRG = (uint8)temp;
             SPBRGH = (uint8)(temp>>8);
             break;
        case EUSART_ASYNC_16BIT_HIGH_SPEED:
             ...
             (TXSTAbits.BRGH = 1);
             (BAUDCONbits.BRG16 = 1);
             temp = (uint16)(((8000000UL) / ((float)(eusart->baudrate)*4)) - 1);
             SPBRG = (uint8)temp;
             SPBRGH = (uint8)(temp>>8);
             break;
        case EUSART_SYNC_8BIT:
             (TXSTAbits.SYNC = 1);
             (BAUDCONbits.BRG16 = 0);
             SPBRG = (uint8)(((8000000UL) / ((float)(eusart->baudrate)*4)) - 1);
             break;
